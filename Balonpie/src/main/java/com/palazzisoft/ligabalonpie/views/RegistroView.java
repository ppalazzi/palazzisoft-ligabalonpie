...
public class RegistroView {
	...
	@Autowired
	public RegistroView(final ParticipanteController participanteControll,
			final ParticipanteValidator participanteValidaror) {
		this.participanteController = participanteControll;
		this.participanteValidator = participanteValidaror;
	}
	...
	public String showForm(
			@RequestParam(value = "participanteId", required = false) Integer participanteId,
		...
		try {
			if (participanteId != null) {
				Participante participante = participanteController
						.obtenerParticipantePorId(participanteId);
				model.addAttribute("participanteCommand", ParticipanteConverter
	...
	public String onSubmit(
		...
		try {
			if (!errors.hasErrors()) {
				participante = this.guardarParticipante(model,
						participanteCommand, viewExit, request);
	...
	private Participante guardarParticipante(ModelMap model,
			ParticipanteCommand participanteCommand, String viewExit,
	...
	private void setParticipanteEnSession(Participante participante,
			HttpServletRequest request) {
		request.getSession().setAttribute("participanteId",
				participante.getId());
	}
}
